...
export class CreatevehiculeComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(): void {
    ...
    this.Date_MEC = this.item.Date_MEC
      ? new Date(this.item.Date_MEC.toString())
      : new Date();
    this.selectDisponibilite.name = this.item.Disponibilite;
    this.selectCarburant.name = this.item.Carburant;
  }
  fullitem() {
    this.item.Date_MEC = this.datepipe
      .transform(this.Date_MEC, 'yyyy-MM-dd')
      ?.toString();
  ...
  AddVehicules() {
    this.fullitem();
    if (this.item.Immatricule?.toString()) {
      this.serice.addVehicules(this.item).subscribe((res) => alert(res));
      var events = {
        title: localStorage.getItem('CIN') + ' AddVehicules',
  ...
  updateVehicules() {
    if (this.item.Immatricule?.toString()) {
      ...
      var events = [
        {
          title: localStorage.getItem('CIN') + ' updateVehicules',
          start: new Date().toString(),
        },
      ];
      this.serice.addcalender(events).subscribe((res) => alert(res));
